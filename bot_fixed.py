...
if not WEBHOOK_BASE:
    raise RuntimeError("Нет WEBHOOK_BASE/RENDER_EXTERNAL_URL. Задай WEBHOOK_BASE в Render (например https://xxx.onrender.com)")
...
def kb_reasons():
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("2️⃣ Тех.поддержка", callback_data="r:2"),
...
def churn_risk(rating: int) -> str:
    # твоя логика оттока (можно поменять значения, если нужно)
    if rating >= 9:
        return "5–10%"
    if rating >= 7:
        return "25–40%"
...
def extract_inn_kpp(text: str) -> Tuple[str, str]:
    ...
    Если нашли числа:
      - 10/12 цифр -> ИНН
      - 9 цифр -> КПП
    Если не нашли вообще цифр -> кладём исходный текст в ИНН, КПП пустой.
    """
    raw = (text or "").strip()
...
async def on_rating(message: types.Message, state: FSMContext):
    ...
    # 9–10: только благодарность + финал. ИНН/КПП НЕ спрашиваем.
    if rating >= 9:
...
async def ask_inn(message: types.Message, state: FSMContext):
    await message.answer(
        ...
        "чтобы мы могли корректно идентифицировать компанию.\n\n"
        "Можно писать в любом формате:\n"
        "«ИНН 770… КПП 770…», «770…/770…», «770… 770…».\n"
        "➡️ Принимаем любой текст, без валидации.",
    )
