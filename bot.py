...
TXT_NO_ASK_REASON = (
    "Очень жаль, что сервис не оправдал ожидания 😔\n"
...
TXT_ID_REQUIRED = (
    "Чтобы мы могли учесть ваш отзыв,\n"
    "ИНН или номер телефона.\n\n"
...
def kb_skip():
    """Reply-клавиатура с кнопкой 'Пропустить'."""
...
async def on_expectations(message: types.Message, state: FSMContext):
    ...
    if txt == "✅ Да":
        ...
        # Убираем reply-клавиатуру с вариантами "Да/Нет"
        await message.answer("Спасибо!", reply_markup=types.ReplyKeyboardRemove())
        # Отправляем вопрос с Reply-кнопкой "Пропустить"
        await message.answer(TXT_YES_ASK_COMMENT, reply_markup=kb_skip())
...
async def on_reason(call: types.CallbackQuery, state: FSMContext):
    ...
    # Убираем inline-кнопки причин, чтобы не нажимали повторно
    await call.message.edit_reply_markup()

    # Если "Другое" — комментарий обязателен, без кнопки "Пропустить"
    if code == "5":
        await state.update_data(comment_required=True)
        await call.message.answer(TXT_OTHER_MANDATORY_COMMENT, reply_markup=types.ReplyKeyboardRemove())
    else:
        await state.update_data(comment_required=False)
        await call.message.answer(TXT_OPT_COMMENT, reply_markup=kb_skip())
...
async def on_comment(message: types.Message, state: FSMContext):
    ...
    # Обработка кнопки ReplyKeyboard "Пропустить"
    if text == "Пропустить":
        if required:
    ...
    # Если комментарий обязателен — не принимаем пустой
...
async def ask_ident(message: types.Message, state: FSMContext):
    await message.answer(TXT_ID_REQUIRED, reply_markup=types.ReplyKeyboardRemove())
